"""
Test the item batch sign API endpoint in drive's core app.
"""
...
def batch_sign_url(item):
    """Return the batch sign url for the given item."""
    return f"/api/v1.0/items/{item.id!s}/batch-sign/"


@override_settings(ALLOW_SIGN_IMPORT_FILE=False)
def test_api_item_batch_sign_setting_disabled():
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_anonymous():
    """Anonymous users should not be allowed to batch sign an item."""
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_authenticated_unrelated():
    """Users unrelated to the item should not be allowed to batch sign it."""
    user = factories.UserFactory()
    item = factories.ItemFactory()
...
def test_api_item_batch_sign_authenticated_reader_or_editor(role):
    """Readers and editors of an item should not be allowed to batch sign it."""
    user = factories.UserFactory()
...
def test_api_item_batch_sign_privileged_mixed_rows(role, settings):
    """
    Administrators and owners should be able to batch sign an item. Emails
    matching an existing user get an access, unknown emails get an invitation,
    and every created sign request triggers a notification email.
    """
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_user_email_case_insensitive():
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_already_shared():
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_inherited_access():
    """
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_already_invited():
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_duplicated_rows():
    """Duplicated emails in the payload should only create one sign request."""
    user = factories.UserFactory()
    item = factories.ItemFactory(users=[(user, "owner")])
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_administrator_cannot_grant_owner():
    """
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_owner_can_grant_owner():
...
def test_api_item_batch_sign_invalid_payload(rows):
    ...
    response = client.post(batch_sign_url(item), {"rows": rows}, format="json")
...
@override_settings(ALLOW_SIGN_IMPORT_FILE=True)
def test_api_item_batch_sign_synchronizes_descendants():
    """
